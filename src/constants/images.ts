...
export const isFileAsNumber = (fullImageUrl: string) => {
    const regex = /([0-9]+).(png|jpg)/gm;
    return fullImageUrl.match(regex)
}
export const isImageInCache = (fullImageUrl: string) => {
    if( isFileAsNumber(fullImageUrl) != null ) return false;
    return (
        isArweaveImage(fullImageUrl)
