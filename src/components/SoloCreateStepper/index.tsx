...
import CustomThemeContext, { CustomThemeContextProvider } from "../../contexts/custom-theme";
...
export const SoloCreateStepper: React.FC<SoloCreateStepperProps> = ({ data }) => {
  ...
  function onCodePaste(event: any) {
    ...
    if (!isValid) {
      toast.error(<Notification title="Invalid code" description="The code must be 6 digits" />);
  ...
  async function handleCreateProfile() {
    ...
    try {
      ...
      const userData: ProfileData = {
        banner: bannerPublicUrl ? bannerPublicUrl : profileBanner ? profileBanner : undefined,
        description: description,
        discord: discord ? discord : undefined,
        email: email,
        image: imagePublicUrl ? imagePublicUrl : profileImage ? profileImage : undefined,
  ...
  useEffect(() => {
    if (data) {
      ...
      setThemeSelected(
        data.theme === "default" ? JSON.stringify(SOLO_THEMES["default"]) : data.theme
      );
    }
  }, [data]);
  ...
  return (
    <section
      className={`my-20 ${isLoading === true ? "pointer-events-none cursor-not-allowed" : ""}`}
    >
      {currentStep === "details" && (
      ...
      {currentStep === "customization" && (
        <div ref={stepsRefs.customization} className="flex items-center flex-col">
          ...
          <p className="font-normal text-sm text-gray-500 my-3">
            You can upload a profile picture and a banner to be displayed on your profile.
          </p>
          ...
          <p className="font-normal text-sm text-gray-500 mt-3 mb-10 text-center">
            In order to make your profile unique, you can choose a theme from our curated list of
            presets or tune every single color to your liking. Go crazy! ^^
          </p>
