...
export const NftCard: React.FC<NftCardProps> = ({ offer, wallet,collectionVerifeyed }) => {
  ...
  const goToNFT = (e: any) => {
    ...
    if (collName?.split("#")[0] == "solo") {
      ...
      if (Boolean(e.ctrlKey || e.metaKey || e.button === 1)) {
        ...
        history.push(
          `${ROUTES.ITEM}/${collectionPath}/${offer.mint}?pk=${offer.escrowPubkeyStr}${walletParam}`
        )
      }
    }
  }
  ...
  useEffect(() => {
    ...
    const getTokenAccount = async() => {
      const tokenAccountsMint = await connection.getTokenLargestAccounts(new PublicKey(offer.mint));
    ...
    >
      <LazyLoad height={200} offset={100}>
        ...
        <div className="flex-initial relative">
          ...
          <div className={`px-4 pb-2 flex justify-between items-end ${(isToken && window.location.pathname.includes('/wallet')) ? 'hidden' : ''}`}>
