...
export const FeedbackView = () => {
  ...
  async function postFeedback(data: any) {
    const response = await fetch(DISCORD_FEEDBACK_URL, {
      method: "POST",
      headers: {
            ...
            <div>
              ...
              >
                <div className="flex flex-col mb-5">
                  <label className="uppercase text-lg sm:text-xl m-5 ">Contact Info</label>
                  <input
                  ...
                  <div className="mx-5 text-red-900">
                    {errors.contact_info?.type === "required" &&
                      "We need this incase we need to get in touch with you! You get it..."}
                  </div>
                  <label className="uppercase text-lg sm:text-xl m-5 ">Feedback</label>
                  <textarea
                    {...register("feedback", {
                      required: "You forgot say what's on your mind...",
